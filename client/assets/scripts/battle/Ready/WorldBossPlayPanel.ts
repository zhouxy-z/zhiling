...
import { EventMgr, Evt_Hide_Home_Ui, Evt_Show_Home_Ui, Goto } from "../../manager/EventMgr";


export class WorldBossPlayPanel extends Panel {

    protected prefab: string = "prefabs/battle/WorldBossPlayPanel";
    ...
    protected async onShow(...arg: any[]) {
        ...
        // HomeLogic.ins.EnterWorldBossScene();
        EventMgr.emit(Evt_Hide_Home_Ui);
        if(Runtime.game)Runtime.game['SetGameSpeed'](1);
                            ...
                            if (item.getChildByName('head').scale.x != 1) {
                                ...
                                tween(item.getChildByName('skill')).to(0.3, { scale: new Vec3(0, 1, 1) }).call(() => {
                                    tween(item.getChildByName('head')).to(0.3, { scale: new Vec3(1, 1, 1) }).call(() => {
                                        thisObj.slot2Tween.set(slot, false);
                                    }).start();
                                }).start();
                            }
    ...
    private WinShow() {
        ...
        this.resultEffect.setCompleteListener(() => {
            thisObj.resultEffect.setAnimation(0, "pvpwinloop", true);
        })
    ...
    private resultShow(){
        ...
        tween(bg).to(0.5, { scale: new Vec3(1, 1, 1) }).call(() => {
            ...
            setTimeout(() => {
                thisObj.resultNode.getChildByName('mask').active = true;
                thisObj.resultNode.getChildByName('battleReport').active = true;
                thisObj.canExit = true;
            }, 0.5)
        }).start();
    ...
    private LoseShow() {
        ...
        let thisObj = this;
        this.resultEffect.setCompleteListener(() => {
            thisObj.resultEffect.setAnimation(0, "pvplostloop", true);
        })
    ...
    //         node.active = false;
    //     })
    //     this.gameSpeed = 1;//this.gameSpeed == 1 ? 1.5 : this.gameSpeed == 1.5 ? 3 : 1;
