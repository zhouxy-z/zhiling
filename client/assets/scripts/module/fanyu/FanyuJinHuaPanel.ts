...
export enum JinHuaType {
    JinHua = 1,
    ChongSui
}

export class FanyuJinHuaPanel extends Panel {
    protected prefab: string = "prefabs/panel/FanyuJinHuaPanel";
    ...
    protected onLoad() {
        ...
        this.jinhuaBtn = this.node.getChildByPath("jinhuaPage/btnNode/jinhuaBtn").getComponent(Button);
        ...
        this.jinhuaBtn.node.on("click", this.onJinHua, this);
    ...
    private initJinHuaData(){
        ...
        SetNodeGray(this.jinhuaBtn.node, true)
    ...
    onAddJinhuaRole() {
        let curRoles = PlayerData.getJinHuaRole(1);
        if(!curRoles || curRoles.length <= 0) return Tips.Show("暂无可进化英雄")
    ...
    async setJinHuaRole(selects: SPlayerDataRole[]) {
        ...
        // this.tweenStep = 2;
        this.jinhuaCfg = CfgMgr.GetRoleQualityUpCfg(this.jinhuaRole.type, this.jinhuaRole.quality, 1);
    ...
    private updateItem(){
        ...
        if(this.tweenStep < 3){
            SetNodeGray(this.jinhuaBtn.node, is_can_jinhua)
        }
    }
    ...
    private onJinHua(btn:Button) {
        Tips.Show("是否确认进化", ()=>{
            let jinhuaData = {
                ...
                data: { role_id: this.jinhuaRole.id, operate_type: 1}
            }
