...
import { CfgMgr, StdAdvister, StdEquityId, StdLuckyShop, StdShopLucky, StdShopLuckyPool } from "../../manager/CfgMgr";
import PlayerData, { } from "../roleModule/PlayerData"
 import {SAdvister,SLuckyContent,SShopContent,SThing} from "../roleModule/PlayerStruct";
...
export class ShopLuckyItem extends Component {
    ...
    private updateAddTime(): void {
        if (!this.std) return;
        if (this.data && this.data.freex1_count > 0) {
    ...
    private async onBtnClick(btn: Button): Promise<void> {
        switch (btn) {
            case this.oneBtn:
                ...
                if (this.data.freex1_count < 1) {
                    if(this.std.shopStd.ConvertToConsumeType && this.std.shopStd.ConvertToConsumeType.length && this.std.shopStd.ConsumeX1Type[0] != this.std.shopStd.ConvertToConsumeType[0]){
                        ...
                        if (!ItemUtil.CheckThingConsumes(this.std.shopStd.ConsumeX1Type, this.std.shopStd.ConsumeX1ItemId, this.std.shopStd.ConsumeX1Cost, true)) {
                            return;
                        }
                    }
            ...
            case this.tenBtn:
                ...
                if (this.data.freex10_count < 1) {
                    ...
                    if(this.std.shopStd.ConvertToConsumeType && this.std.shopStd.ConvertToConsumeType.length && this.std.shopStd.ConsumeX1Type[0] != this.std.shopStd.ConvertToConsumeType[0]){
                        ...
                        if (!ItemUtil.CheckThingConsumes(this.std.shopStd.ConsumeX1Type, this.std.shopStd.ConsumeX1ItemId, [needCount], true)) {
                            return;
                        }
                    }
            ...
            case this.oneHundredBtn:
                ...
                Session.Send({ type: MsgTypeSend.ShopDoLucky, data: { shop_index_id: this.std.shopId, count: 100, convert_count: 0 } });
                break;
            case this.helpBtn:
    ...
    private updateShow() {
        ...
        if(this.std.shopStd.ConsumeX100Type > 0){
            ...
            let item = ItemUtil.CreateThing(this.std.shopStd.ConsumeX1Type[0], this.std.shopStd.ConsumeX1ItemId[0], this.std.shopStd.ConsumeX1Cost[0] * 100);
        ...
        // this.yetNumLab.string = `${Math.max(num - (this.data ? this.data.get_number : 0))}次`;
        this.numLab.string = `${Math.max(num - (this.data ? this.data.get_number : 0))}`;
        this.yetNumLab.string = `${Math.max(this.data ? this.data.do_count : 0)}/${this.std.shopStd.Frequency}`;
        this.updateAddTime();
        if (this.data && this.data.freex10_count > 0) {
