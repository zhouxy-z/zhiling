...
import PlayerData from "../roleModule/PlayerData"
 import {SAssistRoleInfo,SDownlineInfo,SGetAgentInfo,SGetContactInfo,SGetDownlines,SGetIncomes,SIncomeRecordInfo,SIncomeRecords,SIncomesInfo,SPlayerData,SPlayerDataItem,SThing} from "../roleModule/PlayerStruct";
...
export class FrienModule {
    constructor() {
        ...
        Session.on(MsgTypeRet.GetDownlinesRet, this.GetDownlines, this);
        Session.on(MsgTypeRet.GetUplineInfoRet, this.GetUplineInfo, this);
        Session.on(MsgTypeRet.BindUplineRet, this.BindUpline, this);
    ...
    private IncomeResponse(data) {
        if (data.amount) {
            ...
            if(GameSet.GetServerMark() == "xf"){
                reward = [{ type: ThingType.ThingTypeCurrency, currency: {type: 0, value: data.amount} }]
            } 
    ...
    /**获取下线好友 */
    private GetDownlines(data: SGetDownlines) {
