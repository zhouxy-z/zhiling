import { AssetManager, Button, EditBox, EventTouch, Input, Label, Node, Skeleton, Sprite, SpriteFrame, Toggle, Touch, UIOpacity, instantiate, js, path, random, sp, utils, v3 } from "cc";
...
import PlayerData from "../roleModule/PlayerData"
 import {SGetAgentInfo,SGetIncomes,SGetUplineInfo,SIncomesInfo,SUplineInfo,Tips2ID} from "../roleModule/PlayerStruct";
import { ResMgr, folder_item, folder_sound } from "../../manager/ResMgr";
import { FriendHelpPanel } from "./FriendHelpPanel";
import { FriendBindOrUnbindPanel, FriendBindType } from "./FriendBindOrUnbindPanel";
import { Second, ToFixed, formatNumber, randomI, randomf } from "../../utils/Utils";
import { DateUtils } from "../../utils/DateUtils";
import { CfgMgr, StdCommonType } from "../../manager/CfgMgr";
import { AudioGroup, AudioMgr, SoundDefine } from "../../manager/AudioMgr";
...
export class FriendPanel extends Panel {
    protected prefab: string = "prefabs/panel/friend/FriendPanel";
    ...
    async flush(...args: any[]) {
        ...
        if(GameSet.GetServerMark() == "xf"){
            ...
            this.helpFightBtn.node.active = false;
            this.filtrateBtn.node.active = false;
            let res = await ResMgr.LoadResAbSub("sheets/common/lingshi/spriteFrame", SpriteFrame);
            this.incomeNum.node.parent.getChildByName("mark-001").getComponent(Sprite).spriteFrame = res
        }
    ...
    /**刷新收益信息 */
    private async updateIncomeData(data: SGetIncomes | SGetAgentInfo) {
            ...
            if (Income.incomes.length != 0) {
                for (let index = 0; index < this.itemNode.length; index++) {
                    if (datas[index] && datas[index].amount != 0) {
                        ...
                        let item = this.itemNode[index].getChildByName("item");
                        ...
                        let icon = this.itemNode[index].getChildByPath("item/icon").getComponent(Sprite);
                        if(GameSet.GetServerMark() == "xf"){
                            icon.spriteFrame = await ResMgr.LoadResAbSub("sheets/common/lingshi/spriteFrame", SpriteFrame);
                        }
