...
import { CfgMgr, StdPVPSnatch, ThingItemId, ThingType } from "../../manager/CfgMgr";
import { ItemUtil } from "../../utils/ItemUtils";
import PlayerData, { } from "../../module/roleModule/PlayerData"
 import {SMatchPlayerData, SThing} from "../../module/roleModule/PlayerStruct";
import { AutoScroller } from "../../utils/AutoScroller";
import { formatNumber } from "../../utils/Utils";
import { Session } from "../../net/Session";
import { MsgTypeRet, MsgTypeSend } from "../../MsgType";
import { MsgPanel } from "../../module/common/MsgPanel";
import { EventMgr, Evt_BuyPlunderTimes, Evt_Item_Change } from "../../manager/EventMgr";
import { LootVsNumBuyItem } from "./LootVsNumBuyItem";
...
export class LootVsBuyNumPanel extends Panel {
    ...
    private btn:Button;
    private stdPvp:StdPVPSnatch;
    private pvpData:SMatchPlayerData;
    ...
    public flush(selectIndex:number = -1): void{
        this.stdPvp = CfgMgr.getPVPById(PlayerData.LootSeasonInfo.season_id);
        let itemData:SThing = ItemUtil.CreateThing(ThingType.ThingTypeCurrency, 0, this.stdPvp.Money);
        this.datas.push(itemData);
        itemData = ItemUtil.CreateThing(ThingType.ThingTypeItem, this.stdPvp.ConsumeItem, this.stdPvp.ConsumeNumber);
        this.datas.push(itemData);

        this.pvpData = PlayerData.LootPlayerData;
        this.tempCurNum = this.pvpData.match_count;
        this.residueNumLab.string = `${this.pvpData.match_count}次`;
    ...
    protected onShow(): void {
        ...
        EventMgr.on(Evt_BuyPlunderTimes, this.onBuyPvpTimesRet, this);
    }

    protected onHide(...args: any[]): void {
        EventMgr.off(Evt_Item_Change, this.onItemUpdate, this);
        EventMgr.off(Evt_BuyPlunderTimes, this.onBuyPvpTimesRet, this);
    }
    private onItemUpdate():void{
        this.updateConsume();
    }
    private onBuyPvpTimesRet():void{
        this.pvpData = PlayerData.LootPlayerData;
        let addNum:number = this.pvpData.match_count - this.tempCurNum;
        this.tempCurNum = this.pvpData.match_count;
        MsgPanel.Show("抢夺次数+" + addNum);
    ...
    private updateItem(item: Node, data: SThing, index:number):void{
        let bankItem:LootVsNumBuyItem = item.getComponent(LootVsNumBuyItem) || item.addComponent(LootVsNumBuyItem);
        item.getChildByName("select").active = index == this.curSelectIndex;

        bankItem.SetData(this.datas[index], this.pvpData.paid_refresh_count, index == 0 ? this.stdPvp.AddTime : 0);
    }
        ...
        switch(btn){
            case this.btn:
                if(this.curSelectIndex == 0){
                    if(this.pvpData.paid_refresh_count < 1){
                    ...
                    Session.Send({ type: MsgTypeSend.BuyPlunderTimes, data: {times:this.curNum}});
                }else{
                    if (!ItemUtil.CheckThingConsumes([this.curSelectData.type], [this.curSelectData.item.id], [this.curSelectData.item.count.mul(this.curNum)], true)) {
                        return;
                    }
    ...
    private updateShow():void{
        if(this.curSelectIndex == 0){
            this.maxNum = this.pvpData.paid_refresh_count;
        }else{
