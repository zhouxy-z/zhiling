...
import {} from "../roleModule/PlayerData"
 import {SFishingTradePlayerSettlementRecordData,SFishingItem,SThing} from "../roleModule/PlayerStruct";
import { DateUtils } from "../../utils/DateUtils";
import { CfgMgr, FuncValueType, StdFishItem, StdFishTradeShip, ThingItemId, ThingType } from "../../manager/CfgMgr";
import { folder_icon, ResMgr } from "../../manager/ResMgr";
import { AwardItem } from "../common/AwardItem";
import { ItemUtil } from "../../utils/ItemUtils";
import { formatNumber } from "../../utils/Utils";

export class FishTradeLogItem extends Component {
    private phaseLab: RichText;
    private timeLab: Label;
    private bournLab: Label;
    private accidentNameLab: Label;
    private fishTitleLab:Label;
    ...
    protected onLoad(): void {
        ...
        this.bournLab = this.node.getChildByName("bournLab").getComponent(Label);
        this.accidentNameLab = this.node.getChildByName("accidentNameLab").getComponent(Label);
        this.fishTitleLab = this.node.getChildByName("fishTitleLab").getComponent(Label);
    ...
    async SetData(data:SFishingTradePlayerSettlementRecordData) {
        if (!this.hasLoad) await this.loadSub;
        console.log(`第${data.round}期 运鱼记录时间戳 ${data.start_time}`);
        let stdShip:StdFishTradeShip = CfgMgr.GetFishTradeShip(data.ship_id);
        let stdKillShip:StdFishTradeShip;
        let killNameStr:string = "";
        if(data.kill_ship_list){
            for (let index = 0; index < data.kill_ship_list.length; index++) {
                stdKillShip = CfgMgr.GetFishTradeShip(data.kill_ship_list[index]);
                if(killNameStr != "") killNameStr += "、";
                killNameStr += stdKillShip.Name;
            }
        }
        ...
        this.bournLab.string = `目的地：${stdShip.Name}`;
        this.phaseLab.string = `<size=30>第<size=40>${data.round}</size>期</size>`;
        
        this.accidentNameLab.string = killNameStr != "" ? killNameStr : "无";
        this.winFlag.active = false;
        this.loseFlag.active = false;
        this.consumeNumLab.string = formatNumber(data.cost, 2);
        if(data.fish_item && data.fish_item.length > 0){
            ...
            this.fishTitleLab.node.active = this.winFlag.active = this.fishItemCont.active = true;
            ...
            this.fishTitleLab.node.active = this.fishItemCont.active = false;
            this.loseFlag.active = this.fishNoneLab.node.active = true;
           // let rote:number = CfgMgr.GetFishTradeCommon.ScoreItemIdCostWeight;
            let awardList:SThing[] = [];
            awardList[0] = ItemUtil.CreateThing(ThingType.ThingTypeItem, CfgMgr.GetFishTradeCommon.ScoreItemId, data.fish_score_get);
            this.awardList.UpdateDatas(awardList);
        }
    }
    ...
    private updateItem(item: Node, data:SThing) {
        ...
        awardItem.FormatCountCb((num:number)=>{
            let itemData:SThing = awardItem.itemData as SThing;
            //运鱼需要向下取整数值
            if(itemData.item.id == ThingItemId.ItemId_5 || itemData.item.id == ThingItemId.ItemId_16){
                return Math.floor(num).toString();
            }
            return `${ThingItemId[itemData.item.id] ? formatNumber(num, 2) : num}`;
        });
