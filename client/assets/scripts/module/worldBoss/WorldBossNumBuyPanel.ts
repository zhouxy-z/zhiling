...
import { CfgMgr, StdWorldBossComm, ThingItemId, ThingType } from "../../manager/CfgMgr";
import { ItemUtil } from "../../utils/ItemUtils";
import PlayerData, { } from "../roleModule/PlayerData"
 import {SPlayerBossData, SThing} from "../roleModule/PlayerStruct";
import { AutoScroller } from "../../utils/AutoScroller";
import { formatNumber } from "../../utils/Utils";
import { Session } from "../../net/Session";
import { MsgTypeRet, MsgTypeSend } from "../../MsgType";
import { MsgPanel } from "../common/MsgPanel";
import { EventMgr, Evt_Item_Change, Evt_WorldBossChallengeNumUpdate } from "../../manager/EventMgr";
import { WorldBossNumBuyItem } from "./WorldBossNumBuyItem";

export class WorldBossNumBuyPanel extends Panel {
    protected prefab: string = "prefabs/panel/worldBoss/WorldBossNumBuyPanel";
    ...
    private stdWorldBoss:StdWorldBossComm;
    private bossData:SPlayerBossData;
    protected onLoad(): void {
        this.stdWorldBoss = CfgMgr.GetWorldBossComm;
        let itemData:SThing = ItemUtil.CreateThing(ThingType.ThingTypeCurrency, 0, this.stdWorldBoss.CountMoney);
        this.datas.push(itemData);
        itemData = ItemUtil.CreateThing(ThingType.ThingTypeItem, this.stdWorldBoss.CountItemID, this.stdWorldBoss.ConsumeNumber);
        this.datas.push(itemData);
    ...
    public flush(selectIndex:number = -1): void{
        this.bossData = PlayerData.roleInfo.boss_data;
        let free:number = CfgMgr.GetWorldBossComm.FreeCount - this.bossData.free;
        this.residueNumLab.string = `${this.bossData.times + free}次`;
    ...
    private updateItem(item: Node, data: SThing, index:number):void{
        let bankItem:WorldBossNumBuyItem = item.getComponent(WorldBossNumBuyItem) || item.addComponent(WorldBossNumBuyItem);
        item.getChildByName("select").active = index == this.curSelectIndex;

        bankItem.SetData(this.datas[index], this.stdWorldBoss.AddCountMax - this.bossData.paid_times, index == 0 ? this.stdWorldBoss.AddCountMax : 0);
    }
        ...
        switch(btn){
            case this.btn:
                if(this.curSelectIndex == 0){
                    ...
                    Session.Send({ type: MsgTypeSend.BuyBossTimes, data: {times:this.curNum}});
                }else{
                    if (!ItemUtil.CheckThingConsumes([this.curSelectData.type], [this.curSelectData.item.id], [this.curSelectData.item.count.mul(this.curNum)], true)) {
                        return;
                    }
    ...
    private updateShow():void{
        if(this.curSelectIndex == 0){
            this.maxNum = this.stdWorldBoss.AddCountMax - this.bossData.paid_times;
        }else{
